...
const BusinessDashboard = () => {
  ...
  const { availableLeads, purchasedLeads } = useMemo(() => {
    const available = state.leads.filter(lead => 
      lead.status === 'New' && 
      (!lead.purchasedBy?.includes(user?.id || '') && 
      (lead.purchasedBy?.length || 0) < 3)
    );
    
    const purchased = state.leads.filter(lead => 
      lead.purchasedBy?.includes(user?.id || '')
    );

    return { availableLeads: available, purchasedLeads: purchased };
  }, [state.leads, user?.id]);
